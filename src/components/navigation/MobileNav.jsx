...
const MobileNav = ({
  ...
  return (
    <>
      <nav
        className={`main-nav md:hidden fixed top-0 right-0 w-[80%] h-screen invisible opacity-0 pointer-events-none z-[45] smooth-trans nav-position ${
          navIsOpen ? "open" : ""
        }`}
      >
        <ul
      ...
      <div
        className={`absolute md:hidden top-0 left-0 w-full h-screen bg-[rgba(0,0,0,0.6)] z-[40] backdrop-blur-[3px] transition-opacity duration-300 ${
          navIsOpen ? "opacity-100 visible" : "opacity-0 invisible"
        }`}
